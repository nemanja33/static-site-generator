...
def split_nodes_link(old_nodes, is_image=False):
  ...
  for node in old_nodes:
    ...
    secondary_node = TextNode(extracted_link[0], type, extracted_link[1])
    if len(parted_array) > 0:
      primary_node = TextNode(parted_array[0], TextType.PLAIN)
      new_nodes.append(primary_node)
    new_nodes.append(secondary_node)
