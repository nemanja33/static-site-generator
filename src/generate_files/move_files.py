import os
import shutil
get_docs_dir = f"{os.getcwd()}/docs"

def make_docs_dir():
  if (os.path.exists(get_docs_dir) == False):
    os.mkdir(get_docs_dir)

def clean_up():
  _, directories, files = next(os.walk(get_docs_dir))
  clean_up_files(files)
  clean_up_directories(directories)

def clean_up_files(files):
  if (len(files) > 0):
    os.remove(f"{get_docs_dir}/{files[0]}")
    if (len(files[1:]) > 0):
      clean_up_files(files[1:])
     
def clean_up_directories(directories):
  if (len(directories) > 0):
    shutil.rmtree(f"{get_docs_dir}/{directories[0]}")
...
def copy_dirs(from_path, to_path):
  ...
  if (len(directories) > 0):
    for dir in directories:
      ...
      _, sub_dirs, sub_files = next(os.walk(end_path))
      os.mkdir(pl_path)
      if (len(sub_dirs) > 0):
        copy_dirs(end_path, pl_path)
      if (len(sub_files) > 0):
        copy_files(sub_files, end_path, pl_path)
  if (len(files) > 0):
...
def mv_files():
  ...
  copy_dirs(get_static_dir, get_docs_dir)
